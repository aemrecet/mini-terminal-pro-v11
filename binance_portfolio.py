import pandas as pd
from binance_fetcher import get_all_symbols, get_klines, cg_get_ohlc
...
def build_returns_matrix(quote="USDT", interval="1d", limit=365, top_n=30):
    ...
    if not syms:
        # Binance erişimi yok → boş değil, CoinGecko fallback uygulayacağız
        # En bilinenlerden bir alt küme deneyelim:
        fallback_syms = ["BTCUSDT","ETHUSDT","BNBUSDT","ADAUSDT","XRPUSDT","SOLUSDT","DOGEUSDT","MATICUSDT","AVAXUSDT","DOTUSDT"]
        syms = fallback_syms[:top_n]
    ...
    for s in syms:
        ...
        if (df is None) or df.empty:
            # Binance boşsa CG OHLC dene (1d eşdeğeri)
            df = cg_get_ohlc(s, days=limit)
        if (df is None) or df.empty:
            continue
